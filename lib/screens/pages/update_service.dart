...
class UpdateServicesPage extends StatefulWidget {
  ...
  const UpdateServicesPage({Key key, this.service}) : super(key: key);
  @override
  _UpdateServicesPageState createState() => _UpdateServicesPageState();
}
...
type.text=widget.service.type;
location.text=widget.service.location;
...
if(_image!=null) {
  await state.uploadFile(_image).then((imagePath) {
    if (imagePath != null) {
      ...
      state.updateService(services);
    }

  });
}
else {
  services.image=widget.service.image;
  state.updateService(services);
}
